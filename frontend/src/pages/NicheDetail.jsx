...
export default function NicheDetail() {
  ...
  return (
    <div className="page">
      <header className="topbar niche-detail-header">
        <div>
          <Link to="/" className="back-link"><ArrowLeft size={14} /> Voltar aos nichos</Link>
          <h1>{niche.name}</h1>
        </div>
        <button
          ...
          disabled={clearing}
        >
          <Trash2 size={15} /> {clearing ? 'Limpando...' : 'Limpar base de leads'}
        </button>
      </header>
...
function CredentialsTab({ nicheId, onSaved }) {
  ...
  async function handleImport() {
    ...
    setImporting(true);
    try {
      await api.post(`/niches/${nicheId}/credentials/copy`, { sourceNicheId });
      load();
      setShowImport(false);
      setSourceNicheId('');
    } catch (err) {
      ...
      <div style={{ marginBottom: '16px', padding: '12px', background: '#f8fafc', borderRadius: '8px', fontSize: '13px' }}>
        ...
        <div style={{ marginTop: '6px', display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
          {[
            ...
            { key: 'serper', label: 'Serper API', check: (c) => c?.configured },
            { key: 'evolution_api', label: 'Evolution API', check: (c) => c?.configured && c?.base_url },
          ].map((item) => {
      ...
      <ul style={{ fontSize: '13px' }}>
        {credentials.map((c) => (
          <li key={c.id} style={{ marginBottom: '6px' }}>
            ...
            {c.configured && <span style={{ color: '#166534' }}> — API key configurada</span>}
